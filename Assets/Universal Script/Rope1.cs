    ...
    void checkWeight()
    {
        if (Rope2.r2.weight < weight)
        {
            if (Rope2.r2.weight == 0)
            {
                ...
                if (!GameManager.instane.fallDown1)
                {
                    GameManager.instane.Face_1.SetBool("Scary", true);
                    GameManager.instane.anime1.SetBool("Hang", true);
                }
                if (GameManager.instane.D1 < 3f)
                {
                    GameManager.instane.anime1.SetFloat("speed", 5.0f);
                }
                else
                {
                    GameManager.instane.anime1.SetFloat("speed", 3.0f);
                }
        ...
        if (Rope2.r2.weight > weight)
        {
            if (weight == 0)
            {
                ...
                if (!GameManager.instane.fallDown1)
                {
                    GameManager.instane.Face_1.SetBool("Scary", false);
                    GameManager.instane.anime1.SetBool("Hang", false);
                }

                GameManager.instane.anime1.SetFloat("speed", 1.0f);
                WheelRotator.wr.rotator(-1,50);
            }
            if (weight != 0)
            {
